...
namespace vkBasalt
{
    ...
    static bool tryReadKscreenDoctor(DisplayHdrCapabilities& caps)
    {
        ...
        while (fgets(buffer, sizeof(buffer), pipe) != nullptr) {
            ...
            if (line.starts_with("Output:")) {
                ...
                line.find("Peak brightness:") != std::string::npos && line.find("unknown") == std::string::npos) {
                size_t pos = line.find(':');
                if (pos != std::string::npos) {
                    float val = 0;
                    std::from_chars(line.c_str() + pos + 1, line.c_str() + line.size(), val);
        ...
        if (foundAnyHdr) {
            ...
            caps.source               = "kscreen";
            Logger::info(
                "Display HDR capabilities from kscreen-doctor: max="
                + std::to_string(static_cast<int>(caps.maxLuminance))
                + " nits, SDR white=" + std::to_string(static_cast<int>(caps.maxFrameAvgLuminance)) + " nits");
            return true;
        }
        return false;
    }
    ...
    static bool tryReadSysfsEdid(DisplayHdrCapabilities& caps, const std::string& monitorName)
    {
        ...
        while ((entry = readdir(dir)) != nullptr) {
            ...
            if (!(edidData[0] == 0x00 && edidData[1] == 0xFF && edidData[2] == 0xFF && edidData[3] == 0xFF
                  && edidData[4] == 0xFF && edidData[5] == 0xFF && edidData[6] == 0xFF && edidData[7] == 0x00)) {
                continue;
            }
        ...
        Logger::info(
            std::string("Display HDR capabilities from EDID (") + best->name
            ...
            + "min=" + std::to_string(caps.minLuminance) + " nits, " + "HDR=" + (caps.hdrSupported ? "yes" : "no"));
        return true;
    }
