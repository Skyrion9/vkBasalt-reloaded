...
namespace vkBasalt
{
    ...
    uint32_t AutoHdrAnalyzer::findMemoryType(uint32_t typeBits, VkMemoryPropertyFlags properties)
    {
        for (uint32_t i = 0; i < pLogicalDevice->memoryProperties.memoryTypeCount; i++) {
            if ((typeBits & (1 << i))
                && (pLogicalDevice->memoryProperties.memoryTypes[i].propertyFlags & properties) == properties) {
                return i;
            }
        }
    ...
    VkBuffer AutoHdrAnalyzer::createBuffer(
        VkDeviceSize size, VkBufferUsageFlags usage, VkDeviceMemory& memory, VkMemoryPropertyFlags memProps)
    {
    ...
    AutoHdrAnalyzer::AutoHdrAnalyzer(
        ...
        const std::string& monitorName) : pLogicalDevice(pDevice), m_extent(extent), m_imageCount(imageCount)
    {
        ...
        float fallbackWhite =
            (detected.detected && detected.sdrWhitePointNits > 0.0f) ? detected.sdrWhitePointNits : 203.0f;
        float fallbackPeak =
            (detected.detected && detected.peakBrightnessNits > 0.0f) ? detected.peakBrightnessNits : 1000.0f;
        ...
        // 2. Create Buffers
        m_histogramBuffer = createBuffer(
            256 * sizeof(uint32_t), VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
            m_histogramMemory);
        ...
        m_temporalBuffer = createBuffer(
            16, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, m_temporalMemory,
            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
        m_metricsBuffer =
            createBuffer(16, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT, m_metricsMemory);

        // Staging buffer for CPU readback of dynamic HDR metadata
        m_stagingMetricsBuffer = createBuffer(
            16, VK_BUFFER_USAGE_TRANSFER_DST_BIT, m_stagingMetricsMemory,
            VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
        pLogicalDevice->vkd.MapMemory(
            pLogicalDevice->device, m_stagingMetricsMemory, 0, VK_WHOLE_SIZE, 0, &m_mappedMetrics);
        ...
        accBindings[1] = {
            ...
            .stageFlags         = VK_SHADER_STAGE_COMPUTE_BIT,
            .pImmutableSamplers = nullptr};
        VkDescriptorSetLayoutCreateInfo accLayoutInfo = {
            .sType        = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO,
            .pNext        = nullptr,
            .flags        = 0,
            .bindingCount = 2,
            .pBindings    = accBindings.data()};
        pLogicalDevice->vkd.CreateDescriptorSetLayout(
            pLogicalDevice->device, &accLayoutInfo, nullptr, &m_accumulateSetLayout);
        ...
        redBindings[2] = {
            ...
            .stageFlags         = VK_SHADER_STAGE_COMPUTE_BIT,
            .pImmutableSamplers = nullptr};
        VkDescriptorSetLayoutCreateInfo redLayoutInfo = {
            .sType        = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO,
            .pNext        = nullptr,
            .flags        = 0,
            .bindingCount = 3,
            .pBindings    = redBindings.data()};
        pLogicalDevice->vkd.CreateDescriptorSetLayout(
            pLogicalDevice->device, &redLayoutInfo, nullptr, &m_reduceSetLayout);
        ...
        metBindings[0] = {
            ...
            .pImmutableSamplers = nullptr};
        VkDescriptorSetLayoutCreateInfo metLayoutInfo = {
            .sType        = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO,
            .pNext        = nullptr,
            .flags        = 0,
            .bindingCount = 1,
            .pBindings    = metBindings.data()};
        pLogicalDevice->vkd.CreateDescriptorSetLayout(
            pLogicalDevice->device, &metLayoutInfo, nullptr, &m_metricsSetLayout);
        ...
        for (uint32_t i = 0; i < m_imageCount; i++) {
            ...
            redWrites[0]                      = {
                ...
                .pNext            = nullptr,
                .dstSet           = m_reduceSets[i],
                .dstBinding       = 0,
                .dstArrayElement  = 0,
                .descriptorCount  = 1,
                .descriptorType   = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
                .pImageInfo       = nullptr,
                .pBufferInfo      = &histBufInfo,
            ...
            redWrites[2] = {
                ...
                .descriptorType   = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
                .pImageInfo       = nullptr,
                .pBufferInfo      = &metBufInfo,
                .pTexelBufferView = nullptr};
            pLogicalDevice->vkd.UpdateDescriptorSets(pLogicalDevice->device, 3, redWrites, 0, nullptr);

            VkWriteDescriptorSet metWrite = {
                .sType            = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,
                .pNext            = nullptr,
                .dstSet           = m_metricsDescriptorSets[i],
                .dstBinding       = 0,
                .dstArrayElement  = 0,
                .descriptorCount  = 1,
                .descriptorType   = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
        ...
        VkPipelineLayoutCreateInfo accPlInfo = {
            .sType                  = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO,
            .pNext                  = nullptr,
            .flags                  = 0,
            .setLayoutCount         = 1,
            .pSetLayouts            = &m_accumulateSetLayout,
        ...
        pLogicalDevice->vkd.CreateComputePipelines(
            pLogicalDevice->device, pLogicalDevice->pipelineCache, 1, &accCpInfo, nullptr, &m_accumulatePipeline);
        ...
        VkPipelineLayoutCreateInfo redPlInfo = {
            .sType                  = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO,
            .pNext                  = nullptr,
            .flags                  = 0,
            .setLayoutCount         = 1,
            ...
            .pPushConstantRanges    = &redPushRange};
    ...
    AutoHdrAnalyzer::~AutoHdrAnalyzer()
    {
        ...
        if (m_stagingMetricsBuffer)
            pLogicalDevice->vkd.DestroyBuffer(pLogicalDevice->device, m_stagingMetricsBuffer, nullptr);
        if (m_stagingMetricsMemory)
            pLogicalDevice->vkd.FreeMemory(pLogicalDevice->device, m_stagingMetricsMemory, nullptr);
    ...
    void AutoHdrAnalyzer::recordCommands(VkCommandBuffer cmdBuf, VkImageView /*inputImageView*/, uint32_t imageIndex)
    {
        ...
        pLogicalDevice->vkd.CmdBindDescriptorSets(
            cmdBuf, VK_PIPELINE_BIND_POINT_COMPUTE, m_accumulateLayout, 0, 1, &m_accumulateSets[imageIndex], 0,
            nullptr);
        ...
        pLogicalDevice->vkd.CmdPipelineBarrier(
            cmdBuf, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 0, nullptr, 1,
            &accToRedBarrier, 0, nullptr);
        ...
        pLogicalDevice->vkd.CmdBindDescriptorSets(
            cmdBuf, VK_PIPELINE_BIND_POINT_COMPUTE, m_reduceLayout, 0, 1, &m_reduceSets[imageIndex], 0, nullptr);
        ...
        pLogicalDevice->vkd.CmdPushConstants(
            cmdBuf, m_reduceLayout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(float), &dt);
        ...
        pLogicalDevice->vkd.CmdPipelineBarrier(
            cmdBuf, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 1,
            &reduceToCopyBarrier, 0, nullptr);
