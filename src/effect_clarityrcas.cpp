...
#define SPEC(id, field) \
    .specId = (id), .specOffset = offsetof(ClarityRcasSpecData, field), \
    .specSize = sizeof(((ClarityRcasSpecData*) 0)->field)
        ...
        VkColorSpaceKHR colorSpace)
    {
        ...
        mapEntries.push_back(
            {.constantID = 16, .offset = offsetof(ClarityRcasSpecData, step1_x), .size = sizeof(float)});
        mapEntries.push_back(
            {.constantID = 17, .offset = offsetof(ClarityRcasSpecData, step1_y), .size = sizeof(float)});
        mapEntries.push_back(
            {.constantID = 18, .offset = offsetof(ClarityRcasSpecData, step2_x), .size = sizeof(float)});
        mapEntries.push_back(
            {.constantID = 19, .offset = offsetof(ClarityRcasSpecData, step2_y), .size = sizeof(float)});

        specData.colorSpaceMode = static_cast<int32_t>(csm);
        mapEntries.push_back(
            {.constantID = 65535, .offset = offsetof(ClarityRcasSpecData, colorSpaceMode), .size = sizeof(int32_t)});
    ...
    const std::vector<EffectParamDesc>& ClarityRcasEffect::getParamDescs() const
    {
        static const std::vector<EffectParamDesc> params = {
            ...
            {.key        = "clarityRBilateralOffset",
             .label      = "Bilateral Offset",
             .type       = ParamType::Float,
             .defaultVal = 1.5,
             .minVal     = 0.5,
             .maxVal     = 3.0,
             .step       = 0.1,
             .category   = "Sharpening",
             .tooltip = "Multiplier on the bilateral sample offset. Combined with Radius to determine fetch distance. "
                        "Default 1.5.",
             SPEC(1, offset)},

            {.key        = "clarityRBlendMode",
             .label      = "Blend Mode",
             .type       = ParamType::Int,
             .defaultVal = 1.0,
             .minVal     = 0.0,
            ...
            {.key        = "clarityRBlendIfLight",
             .label      = "Blend If Light",
             .type       = ParamType::Int,
             .defaultVal = 220.0,
             .minVal     = 0.0,
             .maxVal     = 255.0,
             ...
             .category   = "Sharpening",
             .tooltip =
                 "Pixels brighter than this value receive reduced sharpening. 255 = sharpen everything. Default 220.",
             SPEC(5, blendIfLight)},

            {.key        = "clarityRcasSharpness",
             ...
             .step       = 0.01,
             .category   = "Sharpening",
             .tooltip    = "AMD Robust Contrast-Adaptive Sharpening amount. Higher = more aggressive micro-detail "
                           "sharpening. Default 0.8.",
             SPEC(6, rcasSharpness)},

            {.key        = "clarityRcasStrength",
             .label      = "RCAS Strength",
             .type       = ParamType::Float,
             .defaultVal = 1.0,
             .minVal     = 0.0,
            ...
            {.key        = "clarityREdgeThreshLow",
             .label      = "Edge Thresh Low",
             .type       = ParamType::Float,
             .defaultVal = 0.05,
             .minVal     = 0.0,
             .maxVal     = 1.0,
             ...
             .category   = "Protection",
             .tooltip =
                 "Lower edge threshold. Contrast below this is suppressed. Lower = more fine detail. Default 0.05.",
             SPEC(8, edgeThreshLow)},

            {.key        = "clarityREdgeThreshHigh",
             .label      = "Edge Thresh High",
             .type       = ParamType::Float,
             .defaultVal = 0.35,
             .minVal     = 0.0,
             .maxVal     = 1.0,
            ...
            {.key        = "clarityREnableFilmGrain",
             ...
             .step       = 1.0,
             .category   = "Film Grain",
             .tooltip = "Perceptual film grain with fine + coarse layers. Breaks up banding and adds natural texture. "
                        "Default on.",
             SPEC(11, enableFilmGrain)},

            {.key        = "clarityRFilmGrainStrength",
