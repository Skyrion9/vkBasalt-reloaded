...
namespace vkBasalt
{
    SimpleComputePass::SimpleComputePass(LogicalDevice* pDevice) : pLogicalDevice(pDevice) {}

    SimpleComputePass::~SimpleComputePass()
    {
        onDestroy();

        if (pipeline != VK_NULL_HANDLE) pLogicalDevice->vkd.DestroyPipeline(pLogicalDevice->device, pipeline, nullptr);
        ...
        LogicalDevice* pDevice, VkDeviceSize size, VkBufferUsageFlags usage, VkDeviceMemory& memory)
    {
        ...
        if (result != VK_SUCCESS) {
            Logger::err("SimpleComputePass: CreateBuffer failed");
        ...
        allocInfo.memoryTypeIndex =
            findMemoryType(pDevice->memoryProperties, memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    ...
    VkBuffer SimpleComputePass::createHostVisibleBuffer(
        VkDeviceSize size, VkBufferUsageFlags usage, VkDeviceMemory& memory, void** mapped)
    {
        VkBufferCreateInfo info = {};
        info.sType              = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
        info.size               = size;
        ...
        allocInfo.memoryTypeIndex      = findMemoryType(
            pLogicalDevice->memoryProperties, memReqs.memoryTypeBits,
    ...
    VkImage SimpleComputePass::createImage(
    ...
    VkImageView SimpleComputePass::createImageView(
        LogicalDevice* pDevice, VkImage image, VkFormat format, VkImageViewType viewType, VkImageAspectFlags aspect)
    {
    ...
    VkImage SimpleComputePass::createImage(
        uint32_t width, uint32_t height, VkFormat format, VkImageUsageFlags usage, VkDeviceMemory& memory)
    {
        return createImage(pLogicalDevice, width, height, format, usage, memory);
    }

    VkImageView SimpleComputePass::createImageView(
        VkImage image, VkFormat format, VkImageViewType viewType, VkImageAspectFlags aspect)
    {
        return createImageView(pLogicalDevice, image, format, viewType, aspect);
    }

    void SimpleComputePass::transitionImageLayout(
        ...
        VkAccessFlags dstAccess)
    {
        ...
        pLogicalDevice->vkd.CmdPipelineBarrier(cmdBuf, srcStage, dstStage, 0, 0, nullptr, 0, nullptr, 1, &barrier);
    }
    ...
    void SimpleComputePass::init()
    {
        ...
        VkResult result =
            pLogicalDevice->vkd.CreateShaderModule(pLogicalDevice->device, &smInfo, nullptr, &shaderModule);
        if (result != VK_SUCCESS) {
            Logger::err("SimpleComputePass: CreateShaderModule failed");
            return;
        }
        ...
        result                                  = pLogicalDevice->vkd.CreateDescriptorSetLayout(
            pLogicalDevice->device, &dslInfo, nullptr, &descriptorSetLayout);
        if (result != VK_SUCCESS) {
            Logger::err("SimpleComputePass: CreateDescriptorSetLayout failed");
            return;
        }
        ...
        if (result != VK_SUCCESS) {
            Logger::err("SimpleComputePass: CreatePipelineLayout failed");
            return;
        }
        ...
        if (result != VK_SUCCESS) {
            Logger::err("SimpleComputePass: CreateComputePipelines failed");
            return;
        }
        ...
        if (result != VK_SUCCESS) {
            Logger::err("SimpleComputePass: CreateDescriptorPool failed");
            return;
        }
        ...
        if (result != VK_SUCCESS) {
            Logger::err("SimpleComputePass: AllocateDescriptorSets failed");
            return;
        }
    ...
    void SimpleComputePass::recordCommands(VkCommandBuffer commandBuffer, uint32_t imageIndex)
    {
        ...
        pLogicalDevice->vkd.CmdBindDescriptorSets(
            commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, pipelineLayout, 0, 1, &descriptorSets[imageIndex], 0,
            nullptr);
        if (pushConstantSize > 0) {
            const void* pc = getPushConstants();
            if (pc)
                pLogicalDevice->vkd.CmdPushConstants(
                    commandBuffer, pipelineLayout, VK_SHADER_STAGE_COMPUTE_BIT, 0, pushConstantSize, pc);
        }
