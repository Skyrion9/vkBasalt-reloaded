...
namespace vkBasalt {
    ...
    std::vector<uint32_t> decompressShader(const CompressedShader& shader) {
        ...
        if (g_shaderDDict) {
            ...
            result = ZSTD_decompress_usingDDict(dctx, spirv.data(), alignedSize,
                                                shader.data, shader.compressedSize, g_shaderDDict);
            ZSTD_freeDCtx(dctx);
        } else {
            result = ZSTD_decompress(spirv.data(), alignedSize,
                                     shader.data, shader.compressedSize);
        }
        ...
        if (result != shader.originalSize) {
            Logger::err("zstd decompression size mismatch: expected " +
                        std::to_string(shader.originalSize) + ", got " + std::to_string(result));
            return {};
        }
        return spirv;
    }
