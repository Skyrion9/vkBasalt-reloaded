...
#define SPEC(id, field) \
    .specId = (id), .specOffset = offsetof(NitCalibrationSpecData, field), \
    .specSize = sizeof(((NitCalibrationSpecData*) 0)->field)
    ...
    const std::vector<EffectParamDesc>& NitCalibrationEffect::getCalibrationParams()
    {
        static const std::vector<EffectParamDesc> params = {
            ...
            {.key        = "sdrWhitePointNits",
             .label      = "SDR White Point (nits)",
        ...
        const std::string& monitorName) : m_pConfigRef(pConfig), m_monitorName(monitorName)
    {
        ...
        for (const auto& p : params) {
            ...
            if (p.key == "sdrWhitePointNits" && detected.detected && detected.sdrWhitePointNits > 0.0f)
                def = detected.sdrWhitePointNits;
            if (p.key == "hdrPeakNits" && detected.detected && detected.peakBrightnessNits > 0.0f)
                def = detected.peakBrightnessNits;
        ...
        mapEntries.push_back(
            {.constantID = 2, .offset = offsetof(NitCalibrationSpecData, autoHdrEnabled), .size = sizeof(int32_t)});

        specData.applyGain = applyGainVal;
        mapEntries.push_back(
            {.constantID = 3, .offset = offsetof(NitCalibrationSpecData, applyGain), .size = sizeof(int32_t)});

        specData.sourceColorSpace = static_cast<int32_t>(getColorSpaceMode(sourceFormat, sourceColorSpace));
        mapEntries.push_back(
            {.constantID = 65534,
             .offset     = offsetof(NitCalibrationSpecData, sourceColorSpace),
             .size       = sizeof(int32_t)});

        specData.destColorSpace = static_cast<int32_t>(getColorSpaceMode(destFormat, destColorSpace));
        mapEntries.push_back(
            {.constantID = 65535, .offset = offsetof(NitCalibrationSpecData, destColorSpace), .size = sizeof(int32_t)});
        ...
        bool willCreateAnalyzer =
            m_hdrAdaptive
            && (autoHdrActive || getColorSpaceMode(sourceFormat, sourceColorSpace) != ColorSpaceMode::SDR_SRGB);
        ...
        if (willCreateAnalyzer) {
            ...
            m_autoHdrAnalyzer = std::make_unique<AutoHdrAnalyzer>(
                pLogicalDevice, imageExtent, inputImages.size(), pConfig, srcCsmInt, analyzerCalibMode, monitorName);
            ...
            pLogicalDevice->vkd.CreateDescriptorSetLayout(
                pLogicalDevice->device, &metLayoutInfo, nullptr, &m_dummyMetricsSetLayout);
            ...
            auto findMemType = [&](uint32_t typeBits, VkMemoryPropertyFlags props) -> uint32_t {
                for (uint32_t i = 0; i < pLogicalDevice->memoryProperties.memoryTypeCount; i++) {
                    if ((typeBits & (1 << i))
                        && (pLogicalDevice->memoryProperties.memoryTypes[i].propertyFlags & props) == props)
                        return i;
                }
                return 0;
            };
            ...
            VkDescriptorPoolCreateInfo poolInfo = {
                ...
                .pNext         = nullptr,
                .flags         = 0,
                .maxSets       = static_cast<uint32_t>(inputImages.size()),
                .poolSizeCount = 1,
                .pPoolSizes    = &poolSize};
            ...
            VkDescriptorSetAllocateInfo allocSetInfo = {
                ...
                .pNext              = nullptr,
                .descriptorPool     = m_dummyMetricsPool,
                .descriptorSetCount = static_cast<uint32_t>(inputImages.size()),
                .pSetLayouts        = layouts.data()};
            pLogicalDevice->vkd.AllocateDescriptorSets(
                pLogicalDevice->device, &allocSetInfo, m_dummyMetricsSets.data());
            ...
            for (size_t i = 0; i < inputImages.size(); i++) {
                VkWriteDescriptorSet write = {
                    ...
                    .pNext            = nullptr,
                    .dstSet           = m_dummyMetricsSets[i],
                    .dstBinding       = 0,
                    .dstArrayElement  = 0,
                    .descriptorCount  = 1,
                    .descriptorType   = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
                    .pImageInfo       = nullptr,
                    .pBufferInfo      = &bufInfoDesc,
    ...
    NitCalibrationEffect::~NitCalibrationEffect()
    {
        if (m_dummyMetricsBuffer)
            pLogicalDevice->vkd.DestroyBuffer(pLogicalDevice->device, m_dummyMetricsBuffer, nullptr);
        if (m_dummyMetricsMemory) pLogicalDevice->vkd.FreeMemory(pLogicalDevice->device, m_dummyMetricsMemory, nullptr);
        if (m_dummyMetricsSetLayout)
            pLogicalDevice->vkd.DestroyDescriptorSetLayout(pLogicalDevice->device, m_dummyMetricsSetLayout, nullptr);
        if (m_dummyMetricsPool)
            pLogicalDevice->vkd.DestroyDescriptorPool(pLogicalDevice->device, m_dummyMetricsPool, nullptr);
    }
    ...
    void NitCalibrationEffect::applyEffect(uint32_t imageIndex, VkCommandBuffer commandBuffer)
    {
        ...
        pLogicalDevice->vkd.CmdPipelineBarrier(
            ...
            VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT | VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 0, nullptr, 0, nullptr, 1,
            &memoryBarrier);
        ...
        if (!m_autoHdrAnalyzer && !m_dummyMetricsInitialized) {
            ...
            float dummyData[4] = {
                ...
                m_pConfigRef->getOption<float>("hdrPeakNits", fallbackPeak) * 0.01f, 1.0f, 0.0f};
            ...
            initBarrier.size                  = 16;
            pLogicalDevice->vkd.CmdPipelineBarrier(
                commandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 0, nullptr, 1,
        ...
        if (m_autoHdrAnalyzer) {
            ...
            pLogicalDevice->vkd.CmdPipelineBarrier(
                commandBuffer, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 1,
        ...
        if (m_autoHdrAnalyzer) {
            ...
            pLogicalDevice->vkd.CmdBindDescriptorSets(
                commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 1, 1, &m_dummyMetricsSets[imageIndex],
                0, nullptr);
        }
