...
namespace vkBasalt
{
        ...
        Config* pConfig)
    {
        ...
        if (needsUniformBuffer && uniformSize > 0) {
            ...
            pLogicalDevice->vkd.CreateDescriptorSetLayout(
                pLogicalDevice->device, &layoutInfo, nullptr, &imageSamplerDescriptorSetLayout);
        ...
        graphicsPipeline = createGraphicsPipeline(
            pLogicalDevice, vertexModule, pVertexSpecInfo, "main", fragmentModule, pFragmentSpecInfo, "main",
            imageExtent, renderPass, pipelineLayout);

        imageDescriptorSets = allocateAndWriteImageSamplerDescriptorSets(
            pLogicalDevice, descriptorPool, imageSamplerDescriptorSetLayout, {sampler},
        ...
        if (uniformBuffer != VK_NULL_HANDLE) {
            ...
            pLogicalDevice->vkd.UpdateDescriptorSets(
                pLogicalDevice->device, static_cast<uint32_t>(uboWrites.size()), uboWrites.data(), 0, nullptr);
    ...
    SimpleEffect::~SimpleEffect()
    {
        ...
        if (uniformBuffer && pLogicalDevice)
            pLogicalDevice->vkd.DestroyBuffer(pLogicalDevice->device, uniformBuffer, nullptr);
        if (uniformMemory && pLogicalDevice)
            pLogicalDevice->vkd.FreeMemory(pLogicalDevice->device, uniformMemory, nullptr);
