...
namespace vkBasalt
{
        ...
        VkColorSpaceKHR colorSpace)
    {
        ...
        static stbi_uc identityLut[2 * 2 * 2 * 4] = {0,   0,   0,   255, 255, 0,   0,   255, 0,   255, 0,
        ...
        if (!fileValid) {
            ...
            usingPNG = static_cast<int32_t>(
                lutFile.find(".cube") == std::string::npos && lutFile.find(".CUBE") == std::string::npos);
            if (!usingPNG) {
                ...
                if (!pixels) {
                    ...
                    Logger::err(
                        "Invalid LUT PNG dimensions (width must equal height * height). Falling back to identity LUT.");
        ...
        VkExtent3D lutImageExtent = {
            .width  = static_cast<uint32_t>(height),
            .height = static_cast<uint32_t>(height),
            .depth  = static_cast<uint32_t>(height)};
        lutImage = createImages(
            pLogicalDevice, 1, lutImageExtent, VK_FORMAT_R8G8B8A8_UNORM,
            VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
            lutMemory)[0];

        uploadToImage(
            pLogicalDevice, lutImage, lutImageExtent, static_cast<uint32_t>(height) * height * height * 4, pixels);

        if (freePixels) {
            stbi_image_free(pixels);
        }

        lutImageView = createImageViews(
            pLogicalDevice, VK_FORMAT_R8G8B8A8_UNORM, std::vector<VkImage>(1, lutImage), VK_IMAGE_VIEW_TYPE_3D)[0];
        ...
        lutDescriptorSet = allocateAndWriteImageSamplerDescriptorSets(
            pLogicalDevice, lutDescriptorPool, lutDescriptorSetLayout, {sampler},
    ...
    const std::vector<EffectParamDesc>& LutEffect::getParamDescs() const
    {
        static const std::vector<EffectParamDesc> params = {
            {.key        = "lutFile",
             ...
             .tooltip = "Path to a color lookup table file. Supports .cube (ReShade-compatible 3D LUT) and .png (tiled "
                        "2D strip). "
