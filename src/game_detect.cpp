...
namespace vkBasalt
{
    ...
    static std::string getWinePrefixGameName()
    {
        ...
        for (int depth = 0; depth < 3; depth++) {
            ...
            if (!dirName.empty() && dirName != "pfx" && dirName != "prefix" && dirName != "drive_c" && dirName != "wine"
                && dirName != "default" && dirName != "dosdevices") {
                return dirName;
            }
    ...
    static GameIdentity resolveGameIdentity()
    {
        ...
        if (id.isWine) {
            ...
            if (id.exeName.empty() || id.exeName == "explorer" || id.exeName.find("wine") != std::string::npos
    ...
    static std::vector<GameEntry> loadRegistry()
    {
        ...
        while (std::getline(f, line)) {
            ...
            GameEntry e;
            if (std::getline(ss, e.id, '|') && std::getline(ss, e.exeName, '|') && std::getline(ss, e.path, '|')
    ...
    {
        std::error_code ec;
        std::filesystem::create_directories(std::filesystem::path(registryPath()).parent_path(), ec);
